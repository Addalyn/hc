...
public static class ServerAbilityUtils
{
    ...
    {
        List<ActorData> evaders = new List<ActorData>();
        foreach (ActorData actorData in GameFlowData.Get().GetActors())
        {
            if (ServerActionBuffer.Get().ActorIsEvading(actorData))
            {
                evaders.Add(actorData);
            }
        }
    ...
    {
        hitTargets.RemoveAll(ServerActionBuffer.Get().ActorIsEvading);
    }
    ...
    public static void RemoveEvadersFromHitTargets(ref Dictionary<ActorData, int> hitTargets)
    {
        ...
        foreach (ActorData evader in evaders)
        {
            hitTargets.Remove(evader);
        }
    }

    public static void RemoveEvadersFromHitTargets(
        ref Dictionary<ActorData, AreaEffectUtils.BouncingLaserInfo> hitTargets)
    {
        List<ActorData> evaders = new List<ActorData>();
        foreach (ActorData actorData in hitTargets.Keys)
        {
    ...
    public static TriggeringPathInfo FindShortestPathCrossingOverSquare(
        ...
        Team moverTeam)
    {
        ...
        foreach (MovementInstance mi in movement.m_movementInstances)
        {
            ...
            for (BoardSquarePathInfo boardSquarePathInfo = mi.m_path;
        ...
        {
            return new TriggeringPathInfo(currentShortestMove.m_mover, triggeringPathSegment);
        }
    ...
    public class AbilityRunData
    {
        ...
        {
            m_sequenceSource = new SequenceSource(null, null);
            m_sequenceSource.SetWaitForClientEnable(true);
            m_abilityResults = new AbilityResults(
                abilityRequest.m_caster,
                abilityRequest.m_ability,
                m_sequenceSource,
                true);
            m_abilityResults_fake = new AbilityResults(
                abilityRequest.m_caster,
                abilityRequest.m_ability,
                m_sequenceSource,
                false);
